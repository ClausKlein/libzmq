...
void worker (int num)
{
    ...
    void *queue = zmq_socket (server_ctx, ZMQ_PULL);
    assert (queue);
    int rc = zmq_connect (queue, "inproc://queue");
    assert (rc == 0);
...
void server ()
{
    ...
    assert (server_ctx);
    // create queue
    void *queue = zmq_socket (server_ctx, ZMQ_PUSH);
    assert (queue);
    int rc = zmq_bind (queue, "inproc://queue");
    assert (rc == 0);
    // start workers
    std::thread w[SERVER_WORKER_COUNT];
    for (int i = 0; i < SERVER_WORKER_COUNT; i++)
        w[i] = std::thread (worker, i);
    // ZMQ_SERVER for client messages
    server_sock = zmq_socket (server_ctx, ZMQ_SERVER);
    assert (server_sock);
    rc = zmq_bind (server_sock, SERVER_ADDR);
    assert (rc == 0);
...
void client (int num)
{
    ...
    for (int i = 0; i < CLIENT_CONNECTION; i++) {
        sock[i] = zmq_socket (ctx, ZMQ_CLIENT);
        assert (sock[i]);
        rc = zmq_connect (sock[i], SERVER_ADDR);
        assert (rc == 0);
        // test connection
        zmq_msg_t msg;
        int v = rand () % 256 + 1;
        message_fill (&msg, v);
        rc = zmq_msg_send (&msg, sock[i], 0);
        assert (rc > 0);
        rc = zmq_msg_init (&msg);
        assert (rc == 0);
        rc = zmq_msg_recv (&msg, sock[i], 0);
        assert (rc > 0);
    ...
    client_ready++;
    while (client_ready < CLIENT_COUNT)
        Sleep (10); // wait while all clients open sockets
    ...
    while (1) {
        ...
        if (recconect == CLIENT_RECCONECT) {
            ...
            zmq_close (sock[n]);
            sock[n] = zmq_socket (ctx, ZMQ_CLIENT);
            assert (sock[n]);
            int rc = zmq_connect (sock[n], SERVER_ADDR);
            assert (rc == 0);
        }
    }
}
